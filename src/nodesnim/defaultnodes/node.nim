...
template nodepattern*(nodetype: untyped): untyped =
  variable = `nodetype`(
    ...
    enter: proc() = discard,
    exit: proc() = discard,
    is_ready: false, pausemode: INHERIT,
    anchor: AnchorRef(x1: 0.0, x2: 0.0, y1: 0.0, y2: 0.0)
  )
...
method draw*(self: NodePtr, surface: SurfacePtr) {.base.} =
  ## Draws node.
  ## This used in the Window object.
  discard
...
method handle*(self: NodePtr, event: Event, mouse_on: var NodePtr) {.base.} =
  ## Handles user input.
  ## This used in the Window object.
  discard
